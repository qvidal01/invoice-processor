...
### Added
- Deterministic extraction pipeline with vendor/amount/date parsing and fallback line items (no external API required).
- OCR engine implementation with PyPDF2-first extraction, pdf2image+pytesseract fallback, preprocessing, and orientation detection.
- File safety guardrails in `InvoiceProcessor` (size limits, allowed extensions, optional trusted directories).
- CLI fails fast on configuration errors with clearer messages.
- New documentation: `ARCHITECTURE.md`, `INSTALL.md`, `IMPLEMENTATION_NOTES.md`; README rewritten to reflect current capabilities.
- Expanded unit test coverage for extractor, OCR guardrails, and processor validation.
